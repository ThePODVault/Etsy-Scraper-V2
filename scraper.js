    ...
    (async () => {
      ...
      if (shopName && shopName !== "N/A") {
        try {
          ...
          const aboutRes = await axios.get(getProxyUrl(aboutUrl));
...
export async function scrapeEtsy(url) {
  ...
  for (let i = 0; i < 3; i++) {
    try {
      result = await parseHtml(url);
      const keyFields = [result.title, result.price, result.listingReviews];
      const missing = keyFields.some((f) => f === "N/A" || f.length === 0);
      if (!missing) return result;
      console.warn(`⚠️ Retry ${i + 1} – Missing fields. Waiting...`);
      await sleep(2000);
    } catch (err) {
